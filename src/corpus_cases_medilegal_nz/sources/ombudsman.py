...
class OmbudsmanSourceAdapter(SourceAdapter):
    """Adapter for Ombudsman reports."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Ombudsman listing records."""
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Ombudsman records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "ombudsman")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Ombudsman records contain the required fields."""
        for case in cases:
