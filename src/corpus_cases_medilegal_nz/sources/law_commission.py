...
class LawCommissionSourceAdapter(SourceAdapter):
    """Adapter for Law Commission reports."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Law Commission listing records."""
        ...
        try:
            ...
            logger.info("Fetched Law Commission listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(source_id="law_commission", url=url, html=response.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Law Commission records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "law_commission")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Law Commission records contain the required fields."""
        for case in cases:
