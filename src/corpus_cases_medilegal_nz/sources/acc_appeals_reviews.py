"""ACC appeals and reviews source adapter."""
...
class AccAppealsReviewsSourceAdapter(SourceAdapter):
    """Adapter for ACC appeal and review material."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Fetched ACC appeals page (%d bytes)", len(response.text))
            return parse_source_specific_listing_html(
                source_id="acc_appeals_reviews",
                url=url,
                html=response.text,
            )
        finally:
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for case in raw_cases:
            case.setdefault("source", "acc_appeals_reviews")
