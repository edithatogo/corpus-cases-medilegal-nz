"""Mental Health Review Tribunal source adapter."""
...
class MentalHealthReviewTribunalSourceAdapter(SourceAdapter):
    """Adapter for Mental Health Review Tribunal material."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Fetched MHRT page (%d bytes)", len(response.text))
            return parse_source_specific_listing_html(
                source_id="mental_health_review_tribunal",
                url=url,
                html=response.text,
            )
        finally:
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for case in raw_cases:
            case.setdefault("source", "mental_health_review_tribunal")
