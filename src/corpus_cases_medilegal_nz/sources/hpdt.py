...
class HpdtSourceAdapter(SourceAdapter):
    """Adapter for HPDT decision documents."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse HPDT listing records."""
        ...
        try:
            ...
            logger.info("Fetched HPDT search page (%d bytes)", len(resp.text))
            return parse_source_listing_html(source_id="hpdt", url=url, html=resp.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure HPDT records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "hpdt")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all HPDT records contain the required fields."""
        for case in cases:
