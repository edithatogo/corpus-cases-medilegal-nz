...
class EraSourceAdapter(SourceAdapter):
    """Adapter for Employment Relations Authority decision documents."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse ERA listing records."""
        ...
        try:
            ...
            logger.info("Fetched ERA search page (%d bytes)", len(resp.text))
            return parse_source_listing_html(source_id="era", url=url, html=resp.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure ERA records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "era")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all ERA records contain the required fields."""
        for case in cases:
