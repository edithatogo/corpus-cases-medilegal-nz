"""Social Security Appeal Authority source adapter."""
...
class SocialSecurityAppealAuthoritySourceAdapter(SourceAdapter):
    """Adapter for Social Security Appeal Authority decisions."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Fetched SSAA page (%d bytes)", len(response.text))
            return parse_source_specific_listing_html(
                source_id="social_security_appeal_authority",
                url=url,
                html=response.text,
            )
        finally:
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for case in raw_cases:
            case.setdefault("source", "social_security_appeal_authority")
