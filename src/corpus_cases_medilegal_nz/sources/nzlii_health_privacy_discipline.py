"""NZLII health, privacy, and discipline subset source adapter."""
...
class NzliiHealthPrivacyDisciplineSourceAdapter(SourceAdapter):
    """Adapter for NZLII health, privacy, and discipline subsets."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Fetched NZLII subset page (%d bytes)", len(response.text))
            return parse_source_specific_listing_html(
                source_id="nzlii_health_privacy_discipline",
                url=url,
                html=response.text,
            )
        finally:
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for case in raw_cases:
            case.setdefault("source", "nzlii_health_privacy_discipline")
