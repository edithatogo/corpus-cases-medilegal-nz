"""Royal Commissions & Waitangi Tribunal source adapter."""
...
class RoyalCommissionsSourceAdapter(SourceAdapter):
    """Adapter for Royal Commissions & Waitangi Tribunal reports."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Royal Commissions listing records."""
        ...
        try:
            ...
            logger.info("Fetched Royal Commissions listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(
                source_id="royal_commissions",
                url=url,
                html=response.text,
            )
        finally:
    ...
    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Royal Commissions records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "royal_commissions")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Royal Commissions records contain the required fields."""
        for case in cases:
