"""Independent Police Conduct Authority source adapter."""
...
class IpcaSourceAdapter(SourceAdapter):
    """Adapter for IPCA reports."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse IPCA listing records."""
        ...
        try:
            ...
            logger.info("Fetched IPCA listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(source_id="ipca", url=url, html=response.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure IPCA records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "ipca")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all IPCA records contain the required fields."""
        for case in cases:
