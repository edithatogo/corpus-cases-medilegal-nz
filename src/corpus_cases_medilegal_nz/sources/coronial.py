...
class CoronialSourceAdapter(SourceAdapter):
    """Adapter for Coronial decisions and findings."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Coronial listings."""
        ...
        try:
            ...
            logger.info("Fetched Coronial listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(source_id="coronial", url=url, html=response.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Coronial records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "coronial")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Coronial records contain the required fields."""
        for case in cases:
