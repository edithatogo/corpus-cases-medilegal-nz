"""Privacy Commissioner source adapter."""
...
class PrivacySourceAdapter(SourceAdapter):
    """Adapter for Privacy Commissioner decisions."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Privacy Commissioner listing records."""
        ...
        try:
            ...
            logger.info("Fetched Privacy Commissioner listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(source_id="privacy", url=url, html=response.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Privacy Commissioner records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "privacy")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Privacy Commissioner records contain the required fields."""
        for case in cases:
