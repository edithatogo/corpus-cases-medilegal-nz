"""Human Rights Commission/Tribunal source adapter."""
...
class HumanRightsSourceAdapter(SourceAdapter):
    """Adapter for Human Rights Commission/Tribunal decisions."""
    ...
    def fetch(self) -> list[dict[str, Any]]:
        """Fetch and parse Human Rights listing records."""
        ...
        try:
            ...
            logger.info("Fetched Human Rights listing page (%d bytes)", len(response.text))
            return parse_source_listing_html(source_id="human_rights", url=url, html=response.text)
        finally:
            session.close()

    def process(self, raw_cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Ensure Human Rights records carry source and metadata containers."""
        for case in raw_cases:
            case.setdefault("source", "human_rights")
            case.setdefault("metadata", {})
        return raw_cases

    def validate(self, cases: list[dict[str, Any]]) -> bool:
        """Return whether all Human Rights records contain the required fields."""
        for case in cases:
